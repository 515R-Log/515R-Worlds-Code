...
class Drive {
 public:
  ...
  struct point_pid_build {
    double x; double y; double speed; bool backwards; point_snap snap; point_transition transition;
  };
  ...
  void enable_odometry();

  void disable_odometry();

  std::vector<point_pid_build> targetPoints;

  double pointPIDSwitch;
   ...
   */
  Drive(std::vector<int> left_motor_ports, std::vector<int> right_motor_ports, int imu_port, double wheel_diameter, double ticks, double ratio, char fwd_top_port, char back_top_port, int extender_port,
  double fwd_wheel_diameter, double back_wheel_diameter, const double fwd_wheel_offset, const double back_wheel_offset);
  ...
  void drive_pid(double target, int speed, bool slew_on = false, bool toggle_heading = true);
  ...
  void turn_pid(double target, int speed=100);
   ...
   *        0 to 127, max speed during motion
   */
  void set_turn_pid(double target, int speed=100);

  void swing_pid(e_swing type, double target, int speed);
  ...
  void wait_until_x(double x);

  void wait_until_y(double y);

  void add_point(double x, double y, double speed=60, bool backwards=false, point_snap snap=SNAP, point_transition pointTransition=LAST_POINT);

  void point_pid(double speed=-1);

  void set_point_pid(double speed=-1);

  void point_pid(double x, double y, double speed=60, bool backwards=false, point_snap snap=SNAP);

  void set_point_pid(double x, double y, double speed=60, bool backwards=false, point_snap snap=SNAP);

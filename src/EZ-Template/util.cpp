...
//
double calcHypo (point2 pointA, point2 pointB){
  double sum = std::pow(pointB.x-pointA.x,2)+std::pow(pointB.y-pointA.y,2);
  double result = std::sqrt(sum);
  return result;
}

double calcAngle (point2 pos, point2 target){
  point2 delta(target.x-pos.x,target.y-pos.y);
  if(delta.y<0.05 && delta.y>=0)
    delta.y=0.05;
  if(delta.y>-0.05 && delta.y<=0)
    delta.y=-0.05;

  double result = std::atan2(delta.x,delta.y);

  return result;
}

double cutMiddle (double value, double cutRange){
  cutRange=fabs(cutRange);

  if(value<cutRange && value>=0)
    value=cutRange;

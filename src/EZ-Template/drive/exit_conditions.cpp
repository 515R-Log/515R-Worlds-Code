...
void Drive::set_exit_condition(int type, int p_small_exit_time, double p_small_error, int p_big_exit_time, double p_big_error, int p_velocity_exit_time, int p_mA_timeout) {
  ...
  if(type == point_turn_exit) {
    pTurnPID.set_exit_condition(p_small_exit_time, p_small_error, p_big_exit_time, p_big_error, p_velocity_exit_time, p_mA_timeout);
  }
}
...
void Drive::wait_until_x(double x) {
  bool start = util::sgn(x-odom.getX());
  while(start == util::sgn(x-odom.getX()))
    pros::delay(util::DELAY_TIME);
}

void Drive::wait_until_y(double y) {
  bool start = util::sgn(y-odom.getY());
  while(start == util::sgn(y-odom.getY()))
    pros::delay(util::DELAY_TIME);
}

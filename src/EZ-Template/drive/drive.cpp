...
Drive::Drive(std::vector<int> left_motor_ports, std::vector<int> right_motor_ports,
  int imu_port, double wheel_diameter, double ticks, double ratio, char fwd_top_port,
  char back_top_port, int extender_port, double fwd_wheel_diameter, double back_wheel_diameter,
  const double fwd_wheel_offset, const double back_wheel_offset)
    : imu(imu_port),
      left_tracker(-1, -1, false),   // Default value
      right_tracker(-1, -1, false),  // Default value
      left_rotation(-1),
      right_rotation(-1),
      odom(fwd_top_port, back_top_port, extender_port, imu_port, fwd_wheel_diameter,
        back_wheel_diameter, fwd_wheel_offset, back_wheel_offset),
      
      ez_auto([this] { this->ez_auto_task(); }){
  is_tracker = DRIVE_INTEGRATED;

  pTurnPID.set_target(0);
...
void Drive::set_defaults() {
  ...
  set_exit_condition(point_turn_exit, 100, 3, 500, 7, 500, 500);
...
double Drive::get_tick_per_inch() {
  CIRCUMFERENCE = WHEEL_DIAMETER * 3.141592657989323;
...
void Drive::imu_loading_display(int iter) {
  ...
  pros::screen::set_pen(0x5d62f4);  // Brand Color
  pros::screen::print(pros::E_TEXT_LARGE,150,70,"515R");
  pros::screen::print(pros::E_TEXT_MEDIUM,150,140,"Revision");

  // Create the metal
  pros::screen::set_pen(0xd6d6d6);
  pros::screen::fill_rect(boarderLeft, boarderY, boarderLeft-20, boarderY+20);
  // Create the eraser
  pros::screen::set_pen(0xfd5386);
  pros::screen::fill_rect(boarderLeft-20, boarderY, boarderLeft-40, boarderY+20);


  // While IMU is loading
  if (iter < 2000) {
    static int last_x1 = boarderLeft;
    // static int last_x1 = boarder;
    pros::screen::set_pen(0x5d62f4);  // Brand Color

    // pros::screen::set_pen(0x00FF6EC7);  // EZ Pink
    int x1 = (iter * ((480 - (boarderRight * 2)) / 2000.0)) + boarderLeft;
    pros::screen::fill_rect(last_x1, boarderY, x1, 240 - boarderY);
  ...
  // Failsafe time
    ...
    // static int last_x1 = boarder;
    pros::screen::set_pen(0xfd5386);
    // pros::screen::    pros::screen::set_pen(0x5d62f4);  // Brand Color
// set_pen(COLOR_RED);
int x1 = ((iter - 2000) * ((480 - (boarderRight * 2)) / 1000.0)) + boarderLeft;
